...
public class Game {
    private static final int INIT_RECEVE_CARD_COUNT = 2;
    ...
    private List<Player> playingPhase(Scanner sc, CardDeck cardDeck, List<Player> players){
        ...
        while (true){
            ...
            if(isAllPlayersTurnOff(cardReceivedPlayers)){
                break;
            }
        }
    ...
    private boolean isAllPlayersTurnOff(List<Player> players){
        for(Player player : players){
            if(player.isTurn()){
                return false;
            }
    ...
    private List<Player> initPhase(CardDeck cardDeck, List<Player> players){
        ...
        for(int i = 0; i < INIT_RECEVE_CARD_COUNT; i++){
           for(Player player : players) {
               System.out.println(player.getName()+"님 차례입니다.");
               Card card = cardDeck.draw();
               player.receiveCard(card);
